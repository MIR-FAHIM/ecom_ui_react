  ...
  const fetchProducts = useCallback(
    async (pg = 0, perPage = 10) => {
      try {
        ...
        const res = await getInactiveProduct(params);
    ...
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      <Stack direction="row" justifyContent="space-between" alignItems="flex-start" flexWrap="wrap" gap={1.5} sx={{ mb: 3 }}>
        <Stack direction="row" alignItems="center" spacing={1.5}>
          ...
          <Box>
            <Typography variant="h5" fontWeight={800} sx={{ lineHeight: 1.2 }}>Inactive Products</Typography>
            <Typography variant="body2" color="text.secondary">
              {loading ? "Loading..." : totalProducts > 0 ? `${totalProducts.toLocaleString()} product${totalProducts !== 1 ? "s" : ""} total` : "Manage your inactive products"}
            </Typography>
          </Box>
        </Stack>
      ...
      <Card sx={{ borderRadius: 2.5, border: "1px solid", borderColor: "divider", mb: 2 }}>
        <CardContent sx={{ p: 2, "&:last-child": { pb: 2 } }}>
          <Stack direction={{ xs: "column", sm: "row" }} spacing={1.5} flexWrap="wrap" useFlexGap alignItems="center">
            <TextField size="small" placeholder="Search by name, SKU or slug..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
        ...
        {loading ? (
          ...
          <Box sx={{ py: 7, textAlign: "center" }}>
            ...
            <Typography variant="h6" fontWeight={600} color="text.secondary">
              {hasFilters ? "No products match the current filters" : "No inactive products yet"}
            </Typography>
