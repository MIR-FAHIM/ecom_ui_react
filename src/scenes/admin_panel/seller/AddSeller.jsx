...
import { registerSeller } from "../../../api/controller/admin_controller/user_controller";

const INITIAL = {
  name: "", email: "", password: "", phone: "",
  shop_name: "", address: "", country: "", state: "", city: "", postal_code: "",
};
...
export default function AddSeller() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const payload = { ...form, user_type: "seller" };
      Object.keys(payload).forEach((k) => { if (payload[k] === "") delete payload[k]; });
      await registerSeller(payload);
      setSnack({ open: true, msg: "Seller created successfully!", severity: "success" });
      setForm(INITIAL);
      setTimeout(() => navigate("/ecom/seller/all"), 1500);
    } catch (err) {
      ...
      setSnack({ open: true, msg: err?.response?.data?.message || "Failed to create seller.", severity: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      <Stack direction="row" alignItems="center" spacing={2} mb={3} flexWrap="wrap" gap={1}>
        ...
        >
          <StorefrontOutlinedIcon sx={{ color: "#6366f1" }} />
        </Box>
        <Box>
          <Typography variant="h5" fontWeight={800}>Add New Seller</Typography>
          <Typography variant="body2" color="text.secondary">Create a seller account with shop details</Typography>
        </Box>
      </Stack>
            ...
            <Stack spacing={2.5}>
              ...
              >
                <Typography variant="body2" fontWeight={700} mb={2.5} color="text.primary"
                  sx={{ display: "flex", alignItems: "center", gap: 1 }}
                >
                  <Box component="span" sx={{ width: 3, height: 16, borderRadius: 4, background: "#10b981", display: "inline-block" }} />
                  Shop Details
                </Typography>
          ...
          <Grid item xs={12}>
            <Stack direction="row" justifyContent="flex-end" spacing={1.5}>
              ...
              <Button
                ...
                sx={{ borderRadius: "8px", px: 4, minWidth: 150 }}
              >
