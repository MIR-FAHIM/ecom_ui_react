...
const HomeP1 = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const p = await getAllProducts({ page: 0, per_page: 24 });
              ...
              <Grid item key={product.id} xs={12} sm={6} md={4} lg={3}>
                <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <CardContent sx={{ flex: 1 }}>
                    <Typography variant="subtitle1" noWrap sx={{ fontWeight: 600, cursor: 'pointer' }} onClick={() => navigate(`/product/${product.id}`)}>{product.name}</Typography>
                  ...
                  <CardActions>
                    <Button size="small" onClick={() => navigate(`/product/${product.id}`)}>View</Button>
